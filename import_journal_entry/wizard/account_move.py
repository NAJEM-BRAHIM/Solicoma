...
class gen_journal_entry(models.TransientModel):
        ...
        if values.get('analytic_distribution'):
            if ';' in  values.get('analytic_distribution'):
                ...
                for name in analytic_distribution_names:
                    distribution= self.env['account.analytic.account'].search([('name', '=', name),('company_id', '=', self.env.company.id)])
                    if not distribution:
    ...
    def import_move_lines (self):
        if  self.import_option == 'csv':
            keys = ['name', 'partner','analytic_distribution', 'account_code', 'date_maturity', 'debit', 'credit', 'amount_currency','currency']  
            try:
