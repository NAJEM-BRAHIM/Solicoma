...
class DamancomDeclaration(models.Model):
    ...
    identif_transfert = fields.Char(string="Identifiant Transfert (CNSS)")
    ...
    def action_load_payslips(self):
        ...
        for payslip in payslips:
            ...
            if version and version.l10n_ma_personnes_charge:
                nb_enfants = version.l10n_ma_personnes_charge
            elif hasattr(employee, 'children'):
                nb_enfants = employee.children or 0
    ...
    def _build_record_b02(self, line):
        ...
        num_imma = int(line.num_immatriculation) if line.num_immatriculation and line.num_immatriculation.strip().isdigit() else 0
        ctr = (num_imma + int(round(line.af_a_reverser * 100)) +
    ...
    def _build_record_b04(self, line):
        num_aff = self.company_id.l10n_ma_cnss_num_affilie
        num_for_ctr = int(line.num_immatriculation) if line.num_immatriculation and line.num_immatriculation.strip().isdigit() else 0
    ...
    def _compute_line_ctr(self, line, is_entrant=False):
        num = int(line.num_immatriculation) if line.num_immatriculation and line.num_immatriculation.strip().isdigit() else 0
        if is_entrant:
            return num + line.jours_declares + int(round(line.salaire_reel * 100)) + int(round(line.salaire_plafonne * 100))
        else:
            rang = SITUATION_RANGS.get(line.situation or '', 0)
            return (num + int(round(line.af_a_reverser * 100)) +
                    line.jours_declares + int(round(line.salaire_reel * 100)) +
                    int(round(line.salaire_plafonne * 100)) + rang)
...
class DamancomDeclarationLine(models.Model):
    ...
    num_immatriculation = fields.Char(string='N Immatriculation', required=True)
