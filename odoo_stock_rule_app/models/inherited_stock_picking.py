...
class InheritedStockPicking(models.Model):
    ...
    def search(self, domain, offset=0, limit=None, order=None, count=False):
        if not self.user_has_groups('odoo_stock_rule_app.group_access_all_location'):
            ...
            domain += [("location_dest_id.user_ids", "=", self.env.user.id)]
        return super().search(domain, offset, limit, order, count)

    @api.onchange('picking_type_id')
    def set_domain_picking_type_id(self):
        for rec in self:
            if not rec.user_has_groups('odoo_stock_rule_app.group_access_all_location'):
                domain = {'domain': {'picking_type_id': [("default_location_src_id.user_ids", "=", self.env.user.id)]}}
                return domain
    ...
    def set_domain_location_dest_id(self):
        for rec in self:
            if not rec.user_has_groups('odoo_stock_rule_app.group_access_all_location'):
                domain = {'domain': {'location_dest_id': [("user_ids", "=", self.env.user.id)]}}
                return domain
